...
Future<void> configureDependencies() async {
  ...
  getIt.registerFactory<AuthRepo>(() => AuthPhoneRepo(getIt<AuthRemote>()));
  getIt.registerFactory<ProfileRemote>(() => ProfileFireRemote(getIt<FirebaseFirestore>()));
  getIt.registerFactory<ProfileRepo>(() => ProfileFireRepo(getIt<ProfileRemote>()));
  getIt.registerFactory<ExamRemote>(() => ExamFireRemote(getIt<FirebaseFirestore>()));
  getIt.registerFactory<ExamRepo>(() => ExamFireRepo(getIt<ExamRemote>()));
  getIt.registerFactory<WorkExamRemote>(() => WorkExamFireRemote(getIt<FirebaseFirestore>()));
  getIt.registerFactory<WorkExamRepo>(() => WorkExamFireRepo(getIt<WorkExamRemote>()));
}
